...
| CPU, memory, and live load | Task Manager, Resource Monitor, `Get-Counter`, CIM | Windows Performance Recorder/Analyzer | Capture only the duration needed; traces may contain paths. Efficiency Mode is EcoQoS, not a RAM-freeing tool. |
...
| Drivers and devices | Device Manager, `Get-PnpDevice`, `Win32_PnPSignedDriver`, recommended/optional Windows Update drivers | PC OEM then GPU/NIC/storage vendor support; Catalog only for a specifically matched package | Avoid generic driver-updater utilities and unofficial download sites. Confirm publisher, device match, Windows version, driver version, and signature. |
