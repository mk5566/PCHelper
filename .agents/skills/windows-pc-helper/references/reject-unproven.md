...
| Service killers (lists that set dozens of services to Disabled, “black viper”-style templates, stop `SysMain`/`WSearch`/`bits`/`wuauserv`/`TrustedInstaller` as a pack) | Breaks Update, Search, Store, printers, and servicing; dependencies are not documented in those lists | Disable one **startup app** the user named, after measuring it |
| Unproven “gaming mode” / FPS scripts (ISLC + timer-res packs, “optimize Windows for gaming” .reg, overlay-killers that stop security processes, NVIDIA/AMD “tweaks” from pastebins) | Placebo or security regression; Game Mode is already a Settings toggle | Settings > Gaming > Game Mode; GPU vendor app; close the measured overlay |
...
| “Ultimate Performance” power plan, hidden High Performance plan, or `powercfg -duplicatescheme` as a generic laptop tweak | Can increase energy use and heat, may not be exposed on Modern Standby systems, and has no guaranteed sustained benefit | Settings > Power & battery > Power mode |
